...
%The datasets available from SNAP's website were used for the simulations in \citep{vernize2015malicious} and the static network simulations shown in \Cref{chap:simulations}.
%\\
\vspace{5mm}
...
To make use of the properties described in \Cref{section:socialvanets}, it is important to choose a mobility pattern that properly represents the way vehicles move on a daily basis in the real world.
...
For reasons explained in \Cref{chap:truman}, relatively short inter-contact times is important for TruMan's functionality.
...
Most of the parameters for the ONE simulator were taken from the article detailing the Working Day Movement Model \citep{ekman2008working}; the most important parameters are shown in \Cref{table:parameters}.
...
The transmission range of nodes varies from simulation to simulation, for reasons explained in \Cref{section:density}.
...
For TruMan, higher densities yield better results, since nodes can construct and update their models of the network faster (this is demonstrated in \Cref{chap:simulations}).
...
In \Cref{table:simdensities}, a few densities for different values of $\rho$, $\eta$ and $\alpha$ are shown. 
...
\Cref{table:citydensities} shows the densities of a few major cities around the world, using $\rho = 10$m for all of them.
...
It can be expected that the model will perform even better in real-world scenarios in cities with even higher densities, which is common, as exemplified by \Cref{table:citydensities}.
...
%\caption{Legend showing the meaning of each color in the following graphs.} 
%\label{fig:legend}
%\end{figure}
...
%\centering
%
%	\includegraphics[width=0.6\linewidth]{images/plots/Network_rA_10.0/new_plots/10_10.png}
%	\caption{$\rho$ = 10m.} 
%	
...
%\centering
%
%	\includegraphics[width=0.6\linewidth]{images/plots/Network_rA_10.0/new_plots/30_10.png}
%	\caption{$\rho$ = 30m.} 
%	
%%\caption{Simulation with 10\% malicious nodes and varying values of $\rho$.}
%\label{fig:random30}
...
%
%\begin{subfigure}{0.6\textwidth}
%\includegraphics[width=0.5\linewidth]{images/plots/Network_rA_10.0/new_plots/50_10.png}
...
\pagebreak
\Cref{fig:random103050} shows the results of simulations running with 10\% of nodes acting maliciously, with communications range varying from 10m to 50m.
...
%\centering
%
%	\includegraphics[width=0.6\linewidth]{images/plots/Network_rA_10.0/new_plots/50_10.png}
%	\caption{$\rho$ = 50m.} 
%	
%%\caption{Simulation with 10\% malicious nodes and varying values of $\rho$.}
%\label{fig:random50}
...
%\begin{figure}
%\centering
%\includegraphics[width=0.5\linewidth]{images/plots/Network_rA_10.0/new_plots/10_10.png}
%\caption{$\rho$ = 10m.} \label{fig:tarjan0}
%\end{figure}
%
%\begin{figure}
%\centering
%\includegraphics[width=0.5\linewidth]{images/plots/Network_rA_10.0/new_plots/30_10.png}
%\caption{$\rho$ = 30m.} \label{fig:tarjan1}
%\end{figure}
%
%\begin{figure}
%\centering
%\includegraphics[width=0.5\linewidth]{images/plots/Network_rA_10.0/new_plots/50_10.png}
%\caption{$\rho$ = 50m.} \label{fig:tarjan2}
%\end{figure}

% taking over 8000 iterations with 10m range and achieving solid results at just over 1000 iterations with 50m range.

\pagebreak
\Cref{fig:randommalicious1} and \Cref{fig:randommalicious2} show the variation of results for different amounts of malicious nodes in the network.
...
\pagebreak
All simulations were performed with trust threshold $h=0.5$, except for the ones in \Cref{fig:randomthresholds}, performed to demonstrate the impact of different threshold values.
...
\Cref{fig:random7} shows the execution of the algorithm over the course of 7 days.
...
\pagebreak
\Cref{fig:extramaliciousaging2} shows how TruMan reacts to one node converting from benign to malicious.
...
Eight desired properties for VANET trust models were presented in \Cref{section:properties}.
Here, TruMan's ability to satisfy each property is evaluated.
\Cref{table:properties_truman} replicates the one from \Cref{section:properties} with the addition of TruMan, providing a full comparison between itself and the related work.
